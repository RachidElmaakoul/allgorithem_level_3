...
void PrintEachColsInArray(int arr[3][3],short Rows, short cols) {
	cout << "\nThe the following are the sum of each col in the matrix:\n";
	for (short i = 0; i < cols; i++)
	{
		cout << "col " << i + 1 << " sum = " << setw(3) << SumColumn(arr,Rows,i) << endl;
	}
...
void PrintEachColsInArray(int arrSum[3],short cols) {
	cout << "\nThe the following are the sum of each col in the matrix:\n";
	for (short i = 0; i < cols; i++)
	{
...
int main()
{
	...
	SumColumn(arr, 3, 3);
	PrintEachColsInArray(arr, 3, 3);


	cout << endl;
	sumEachColsInMatrix(arr,arrSum, 3, 3);
	PrintEachColsInArray(arrSum, 3);
